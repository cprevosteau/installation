...
test_docker_src() {
  ...
  (
    ...
    docker run --privileged -itv "${INSTALLATION_DIR}:${INSTALLATION_DIR}:ro" -v "/tmp/archives:/var/cache/apt/archives" "$ENCRYPTED_IMAGE" bats "${tests_dir}/src/${src_file}.bats" "${option}"
  )
}
...
test_docker_real_install() {
  ...
  (
    ...
    docker run -itv "${INSTALLATION_DIR}:${INSTALLATION_DIR}:ro" \
        ...
        "$ENCRYPTED_IMAGE" bats --tap  "${tests_dir}/real_install/${src_file}.bats"
  )
}
...
test_docker_helpers() {
  ...
  (
    . "${src_dir}/utils/import_src_and_env.bash" && import_src_and_env
    docker run -itv "${INSTALLATION_DIR}:${INSTALLATION_DIR}:ro" "$ENCRYPTED_IMAGE" bats "${helpers_dir}/tests/${src_file}.bats" "${option}"
  )
}
